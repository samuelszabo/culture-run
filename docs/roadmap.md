...
## Verzia 2.6 — Dubaj (Burj Khalifa, beh v oblakoch) ✅

Plný dizajn: `docs/level-dubai.md`. Najväčšia zmena doteraz — **hráč nebeží po
rovine, ale skáče ponad medzery v oblakoch** vysoko nad mestom; z oblakov trčia
**iba špičky mrakodrapov**, dookola sa klenú **dúhy**.

- [x] Nová krajina + level: **Dubaj — Burj Khalifa** (`burj-khalifa`, krajina `dubai`, 🇦🇪) — registry, prepnuteľné 3D prostredie
- [x] Mechanika oblakov: nový typ prekážky `cloud-gap` (jumpable — smrteľná **iba na zemi**, preskoč skokom) + špičky mrakodrapov `tower-top` na uhýbanie; logika ostáva 2D, znovupoužitý existujúci skok (overené simuláciou — clean 0-smrtí beh v `tests/level-dubai`)
- [x] Prostredie: biela oblaková cesta s medzerami (diery = modrá obloha pod nohami), špičky mrakodrapov, dúhové oblúky, zlaté slnko, oblačné more; `cloudHeight(trackY)`
- [x] Pamiatky: Burj Khalifa (828 m), Burj Al Arab, Dubai Frame — trčia z oblakov
- [x] Zber: dubajská čokoláda, datle, luqaimat (album fakty pravdivé)
- [x] Odmeny: sokolík-spoločník (5★), dúhové krídla (750 b), dúhový chvost (950 b), **hrateľný Unicorn** (1050 b)
- [x] Hrateľná postavička **Unicorn** (vzor `playable-bear`: odmena odomkne `unicorn` v `COUNTRY_CHARACTERS.dubai`; znovupoužíva quadruped animácie mačky)
- [x] Kvíz (4 otázky) + album, `maxScore = 1080` (parita s Čínou/Slovenskom, assert v testoch)
- [ ] Overenie 60 fps na reálnom mobile a nasadenie (Cloudflare Pages)
